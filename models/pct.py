...
class PCTransformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.calc_dim = config.calc_dim

        self.grouper = DGCNN_Grouper(downsample=self.num_group)

        self.pos_embed = nn.Sequential(
            nn.Conv1d(self.in_chans, 128, 1),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(negative_slope=0.2),
        ...
        self.reduce_map = nn.Linear(self.calc_dim, self.embed_dim)
        self.reduce_map_2 = nn.Linear(384*2, 384)
    ...
    def forward(self, inpc):
        ...
        # print(coor.shape, f.shape) # torch.Size([32, 3, 128]) torch.Size([32, 128, 128])
        ...
        B, M, C = x.shape
        # print(x.shape) # torch.Size([64, 32, 384])
        global_feature = self.increase_dim(x.transpose(1, 2))
        global_feature = torch.max(global_feature, dim=-1)[0]
        # print(global_feature.shape) # torch.Size([64, 1024])
        coarse_point_cloud = self.coarse_pred(global_feature).reshape(B, -1, 3)
        # print(coarse_point_cloud.shape) # torch.Size([64, 32, 3])

        rebuild_feature = torch.cat([
            global_feature.unsqueeze(-2).expand(-1, M, -1),
            x,
            coarse_point_cloud], dim=-1)
        # print(rebuild_feature.shape) # torch.Size([64, 32, 1411])
        rebuild_feature = self.reduce_map(rebuild_feature.reshape(B * M, -1)).reshape(B, M, -1)
        # print(rebuild_feature.shape) # torch.Size([64, 32, 384])
        # cdist_feature = torch.max(rebuild_feature, dim=1)[0]
        cdist_feature = F.adaptive_max_pool1d(rebuild_feature, 1).view(B, -1)

        return coarse_point_cloud, rebuild_feature, cdist_feature
