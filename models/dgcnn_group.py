...
class DGCNN_Grouper(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(coor.shape, f.shape) # torch.Size([48, 3, 2048]) torch.Size([48, 8, 2048])
        f = self.get_graph_feature(coor, f, coor, f)
        f = self.layer1(f)
        f = f.max(dim=-1, keepdim=False)[0]

        # print(coor.shape, f.shape) # torch.Size([48, 3, 2048]) torch.Size([48, 32, 2048])
        coor_q, f_q = self.fps_downsample(coor, f, 512)
        # print(coor_q.shape, f_q.shape) # torch.Size([48, 3, 512]) torch.Size([48, 32, 512])
        ...
        coor_q, f_q = self.fps_downsample(coor, f, self.num_group)
        f = self.get_graph_feature(coor_q, f_q, coor, f)
        f = self.layer4(f)
        f = f.max(dim=-1, keepdim=False)[0]
        coor = coor_q
