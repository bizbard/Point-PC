...
class DGCNN(nn.Module):
    ...
    def get_graph_feature(coor_q, x_q, coor_k, x_k):
        ...
        with torch.no_grad():
            ...
            # print(idx.shape) torch.Size([64, 4, 64])
        ...
        # print(feature.shape, x_q.shape) torch.Size([64, 128, 64, 4]) torch.Size([64, 128, 64, 4])
    ...
    def forward(self, f, coor):
        ...
        # print(coor.shape) torch.Size([64, 3, 64])
        f = f.transpose(1, 2).contiguous()  # B C N
        f = self.input_trans(f)  # B 128 N
        # print(f.shape) torch.Size([64, 128, 64])
...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # print(center.shape) # torch.Size([64, 64, 3])
        # knn to get the neighborhood
        # _, idx = self.knn(xyz, center) # B G M
        idx = knn_point(self.group_size, xyz, center)  # B G M
        # print(idx.shape) # torch.Size([64, 64, 32])
...
class Decoder(nn.Module):
    def __init__(self, encoder_channel, num_fine):
        ...
        b = torch.linspace(-0.05, 0.05, steps=self.grid_size, dtype=torch.float).view(self.grid_size, 1).expand(
            self.grid_size, self.grid_size).reshape(1, -1)
        # print(a.shape, b.shape) # torch.Size([1, 4]) torch.Size([1, 4])
...
class DiscreteVAE(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.dgcnn_1 = DGCNN(encoder_channel=self.encoder_dims * 2, output_channel=self.num_tokens)
        ...
        self.cls = Classifier(feature_channel=self.encoder_dims, num_class=55)
    ...
    def forward(self, inp, fea, temperature=1., hard=False, **kwargs):
        pred = self.cls(fea)
        neighborhood, center = self.group_divider(inp)
        # print(neighborhood.shape, center.shape) # torch.Size([64, 64, 32, 3]) torch.Size([64, 64, 3])
        logits = self.encoder(neighborhood)  # B G C
        # print(logits.shape) # torch.Size([64, 64, 256])
        logits = torch.cat([logits, fea.unsqueeze(1).expand(-1, 64, -1)], dim=-1)
        logits = self.dgcnn_1(logits, center)  # B G N
        # print(logits.shape) # torch.Size([64, 64, 8192])
        soft_one_hot = F.gumbel_softmax(logits, tau=temperature, dim=2, hard=hard)  # B G N
        # print(soft_one_hot.shape) # torch.Size([64, 64, 8192])
        sampled = torch.einsum('b g n, n c -> b g c', soft_one_hot, self.codebook)  # B G C
        # print(sampled.shape) # torch.Size([64, 64, 256])
        feature = self.dgcnn_2(sampled, center)
        # print(feature.shape) # torch.Size([64, 64, 256])
        coarse, fine = self.decoder(feature)
        # print(coarse.shape, fine.shape) torch.Size([64, 64, 8, 3]) torch.Size([64, 64, 32, 3])
        ...
        ret = (whole_coarse, whole_fine)
        # ret = (neighborhood, center)
        # print(ret[0].shape, ret[1].shape, ret[2].shape, ret[3].shape, ret[4].shape, ret[5].shape)
        # torch.Size([64, 512, 3]), torch.Size([64, 2048, 3]), torch.Size([64, 64, 8, 3]), torch.Size([64, 64, 32, 3]), torch.Size([64, 64, 32, 3]), torch.Size([64, 64, 8192])
        return pred, ret